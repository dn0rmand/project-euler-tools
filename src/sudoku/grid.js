module.exports = function (grid) {
  ...
  let generatePossibilites = function (grid) {
    ...
    grid.forEach((main) => {
      ...
      main.forRow((cell) => {
        if (cell.value > 0) main.remove(cell.value);
      });
      main.forColumn((cell) => {
        if (cell.value > 0) main.remove(cell.value);
      });
      ...
      if (main.possibilities.length === 0) grid.isValid = false;
    });
  };
  ...
  let g = {
    ...
    forOtherSquaresV: function (x, y, fn) {
      ...
      for (let y2 = 0; y2 < 9; y2++)
        for (let x2 = xx; x2 < xx + 3; x2++) {
          let cell = cells[y2][x2];
          if (cell.square !== id) if (fn(cell) === false) return;
        }
    },
    ...
    saveState: function () {
      ...
      this.forEach((cell) => {
        if (cell.value === 0) state.push(cell);
      });
    ...
    restoreState: function (state) {
      for (let i = 0; i < state.length; i++) state[i].value = 0;

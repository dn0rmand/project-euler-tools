const bigIntHelper = function () {
  ...
  BigInt.prototype.modPow = function (exp, mod) {
    if (mod == 0) throw new Error("Cannot take modPow with modulus 0");
    ...
    if (base == BigInt.ZERO) return BigInt.ZERO;

    while (exp > BigInt.ZERO) {
      if ((exp & BigInt.ONE) == BigInt.ONE) r = (r * base) % mod;
      exp = exp >> BigInt.ONE;
      if (exp) {
        base = (base * base) % mod;
      }
